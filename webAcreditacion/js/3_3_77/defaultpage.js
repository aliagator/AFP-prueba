 ...
 */
 ...
 // Distinguir si se est� operando con MSIE, FF, etc
 ...
 *
 *	Para usarse contra el evento onKeyPress, permitiendo que solo se acepte
 *	el ingreso al control de d�gitos, el caracter separador de decimales
...
function openKendoWindow(url, title, height, width, additionalOptions, dialogArguments, modal, onCloseFunction) {
    ...
    options = {
        ...
        iframe: true,
        modal: modal,
        close: function() {onKendoWindowClose(container, onCloseFunction);}
    };
...
function onKendoWindowClose(container,onCloseFunction) {
     ...
     window.top.$(container).data("kendoWindow").destroy(); // destroy the window
     window.top.$(container).empty(); // emnopty
     window.top.$(container).remove(); // emnopty
     sntHideClock();
}
