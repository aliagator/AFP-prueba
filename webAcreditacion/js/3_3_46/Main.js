	...
	if ($("#iframe_body")[0]) {
		var pos = $("#iframe_body").offset();
		 ...
		 if (GetCookie("SondaNetMenu")) {
			 if (GetCookie("SondaNetMenu") == "MenuVertical") {
...
function rf_V_click() {
	...
	SetCookie("SondaNetMenu", "MenuHorizontal");
...
function rf_H_click() {
	...
	SetCookie("SondaNetMenu", "MenuVertical");
	setTimeout(doRenderMenu,1);
}
...
function onSubMenuOpen(e) {
	...
	var mnu = e.item.children[1].firstChild.innerText;
