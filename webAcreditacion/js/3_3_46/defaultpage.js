...
if (typeof($) != "function") {
  alert('Biblioteca JQuery no pudo ser cargada o no esta activa');
    ...
    // Invertir un string: e.g. "hola".reverse() -> "aloh"
...
// Captura de eventos onload para rutinas de ayuda
...
function menu_onload(){
	...
	// página. Esto ya que bajo ciertas condiciones los menus no se recargan
	if (parent) {
		try {
			fr = parent.frames;
			var i = 0;
 ...
 *	DY 20080111
 *
 *  Escapa todos los caracteres html de composición (i.e. <, > y &) de manera
 *  de evitar la "excepción" 'Se detectó un posible valor Request.Form
...
function rutValidar(val){//para RutSnd
	...
	//var rv	  = $('#' + val.controltovalidate).RV;
    var rv = document.all[val.controltovalidate].RV.toLowerCase();
	
	valor = valor.replace(/\s/g, '');

	// si no es requerido, puede estar vacío
	if (valor.length == 0) {
		return rv == 'false';
	}

	// si no está vacío, entonces se valida via regex
 ...
 * tamaño y ubicación de la ventana flotante acorde a las dimensiones de su
 * contenido, pero sólo para la dimensión en que se haya especificado cero
 */
function autoAjustarTamanoVentana(w, h) {
	// Se determina ancho, alto, top y left óptimos
 ...
 *	  UniqueID:		   ID del control al que se hará postback en el
...
function asignarControlesPadre(dialogArguments, arrControles, arrValores, UniqueID) {
	...
	for (var i = 0; i < arrControles.length; i++) {
		...
		if (!ctl) {
			alert('control "' + arrControles[i] + '" no pudo ser encontrado ' +
			 'en la pagina padre');
			return false;
		}
...
/*
 * Enfoque "seguro" de control (incluso cuando está disabled)
 */
...
function numeroFocus(control, sepMil, sepDec, numDec, ingresando) {
	...
	if (ingresando) {
		...
		/* Se invierte el string que representa al numero, asi es mas facil
		   operar sobre éste para agregar los separadores de miles. Ademas se
 ...
 *  el ingreso al control de dígitos y el caracter separador de decimales
 *  (solo si numDec > 0)
 *
...
function numeroKeypress(evento, control, sepDec, numDec) {
	...
	/* Se aceptan las pulsaciones del separador de decimales que se haya
	   parametrizado en sepDec, siempre que no haya otro presente */
...
/*
 * Obtiene la posición del cursor dentro de un control textbox o textarea
 */
function posicionCursor(control) {
	if (control.createTextRange){
		cursor = document.selection.createRange().duplicate();
		cursor.moveEnd('character', control.value.length);
		if (cursor.text == '') return control.value.length;
		return control.value.lastIndexOf(cursor.text);
	} else return control.selectionStart;
}
...
function numeroAdvFocus(control, tamano, numDec, sepMil, sepDec, ingresando) {
	...
	if (ingresando) {
		...
		// Si el numero parte con el separador decimal, se le añade un 0
		if (salida.charAt(0) == sepDec) salida = '0' + salida;

		// Reconocimiento de parte entera y decimal
		partes = /-?(\d*)\D?(\d*)/.exec(salida);
		if (partes == null) {
			alert("Se ha ingresado un formato numérico erróneo.");
			try {
				control.focus()
			} catch (e) { }
			return false;
		...
		// Se añaden 0 a la derecha de la parte decimal (si es necesario)
...
/*
 *	Funcion para evento - numeroAdvKeyPress - FALTA NSS (el cual debe usar
 *	onkeydown para cancelar)
 *	DY 20080108
 *
 *	Para usarse contra el evento onKeyPress, permitiendo que solo se acepte
 *	el ingreso al control de dígitos, el caracter separador de decimales
 *	(solo si numDec > 0) y el signo -
 *
 *	  evento	= evento "burbujeado" desde la pagina. Debe usarse "event"
 *	  control   = control contenedor del número (textbox)
 *	  tamano	= cantidad de dígitos que soportará el control (0 = infinito)
 *	  numDec	= cantidad de decimales a considerar
 *	  sepDec	= caracter separador de decimales
 *	  aceptaNeg = indica si acepta negativos
 *
 *  Utilizar en MSIE en evento onKeyPress de la manera:
 *  <input ... onKeyPress="return numeroAdvKeyPress(event, this, ',', 2)" />
 */

function numeroAdvKeyPress(evento, control, tamano, numDec, sepDec, aceptaNeg) {
	var key = (MSIE) ? window.event.keyCode : evento.which;
	...
	// Se aceptan las pulsaciones de los digitos
	if (key >= 48 && key <= 57) {
		var nuevo_value = control.value.slice(0, posicionCursor(control)) +
		  String.fromCharCode(key) +
		  control.value.slice(posicionCursor(control));

		// pero validando la cantidad de enteros y decimales
		partes = /-?(\d*)\D?(\d*)/.exec(nuevo_value);
		ok_enteros = (tamano == 0) || (tamano > 0 &&
		  partes[1].length <= (tamano - numDec));
	...
	if (aceptaNeg) {
		...
		// tal símbolo al comienzo del string
		if (key == 45) {
	...
	// Tambien se acepta la pulsación de . (para poder utilizar el numpad),
	// el cual se reemplazará por el separador decimal sepDec
	if (numDec > 0 && key == 46) {
 ...
 *	DY 20080908
 *
 *	Limita que se pegue desde el clipboard sólo un número que respete el
 *	formato.
 *
 *	  control   = control contenedor del número (textbox)
 *	  tamano	= cantidad de dígitos que soportará el control (0 = infinito)
...
function numeroAdvPaste(control, tamano, numDec, sepMil, sepDec, aceptaNeg) {
	// obtención de texto desde el clipboard
	...
	// cálculo de cantidad de enteros permitidos
	var numEnt = numDec > 0 ? (tamano - numDec) : tamano;
	if (tamano == 0) numEnt = 99;

	// Formación de expresión regular que cuadre
	var s_re = "^";
	s_re += aceptaNeg ? "(-?" : "(";
	s_re += "\\d{1," + numEnt + "})";
	...
	if (!re.test(texto)) {
		alert("Numero siendo pegado (" + texto + ") no se ajusta al\n" +
...
function reloadIFrame(iframe,url) {
	...
	if (!url.match(/-=ABORT=-/)) {
		...
		/* Distintas alternativas para ubicar el frame que contiene el contenido,
		 * por orden de disponibilidad (anterior% era historica%) */
		...
		if (fbody) {
			...
			alert("La funcion reloadIFrame no pudo encontrar el frame interno '" +
			 iframe + "'");
		}
	}
}
