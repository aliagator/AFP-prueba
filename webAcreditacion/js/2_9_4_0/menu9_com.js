...
function Initiate(){
	if(IniFlg&&Ldd){Init(FrstCntnr);IniFlg=0;if(RememberStatus)Rmbr(FrstCntnr);if(ShwFlg)AfterCloseAll();ShwFlg=0;showAllWindowed()}}
...
function GoTo(){
	...
	if(HP.Arr[1]){status=statusText;LowItem(HP);IniFlg=1;Initiate();
...
function Unfld(){
	...
	var TS=ExpYes?ScLoc.document.body.scrollTop:ScLoc.pageYOffset,LS=ExpYes?ScLoc.document.body.scrollLeft:ScLoc.pageXOffset,CCnt=Nav4?P.LLyr.CCn:P.CCn,THt=Nav4?P.clip.height:parseInt(P.style.height),TWt=Nav4?P.clip.width:parseInt(P.style.width),TLt=AcrssFrms&&P.Lvl==1&&!FirstLineHorizontal?0:Nav4?P.Ctnr.left:parseInt(P.Ctnr.style.left),TTp=AcrssFrms&&P.Lvl==1&&FirstLineHorizontal?0:Nav4?P.Ctnr.top:parseInt(P.Ctnr.style.top);
	var CCW=Nav4?P.LLyr.CCn.clip.width:parseInt(P.CCn.style.width),CCH=Nav4?P.LLyr.CCn.clip.height:parseInt(P.CCn.style.height),CCSt=Nav4?P.LLyr.CCn:P.CCn.style,SLt=AcrssFrms&&P.Lvl==1?CCnt.OrgLeft+TLt+LS:CCnt.OrgLeft+TLt,STp=AcrssFrms&&P.Lvl==1?CCnt.OrgTop+TTp+TS+CurrentStartTop-StartTop:CCnt.OrgTop+TTp;
...
function CreateMenuStructure(MNm,No,Mcllr){
	...
	if(MNm=="Menu") { divMen.style.height = 0; }
	for(i=1;i<No+1;i++){WMnu=MNm+eval(i);NOs=eval(WMnu+"[3]");
		...
		PMb=Mbr;
		// Añadido por el tema del scroll
		if(MNm=="Menu") { divMen.style.height = (parseInt(divMen.style.height) + parseInt(Mbr.style.height) + 1) + P_X; }
		}
	MbC.FrstMbr=Mbr;
	RLvl--;
	
	// Añadido por el tema del scroll
	if(MNm=="Menu"){ scroll_RePos();do_scroll(0);}
...
///////// Scroll
var scroll_nav = (document.layers) ? true : false;
var scroll_iex = (document.all) ? true : false;
var scroll_pos = 0;   // initial top position
var scroll_stp = 10;    // step increment size
var scroll_spd = 50;   // speed of increment
var scroll_upr = -390;  // upper limiter
var scroll_lwr = 0;   // lower limiter
var scroll_tim;         // timer variable
var scroll_flagimg = 0; // para evitar redibujados inecesarios
...
function scroll_up() {
if(scroll_pos > scroll_upr) scroll_pos -= scroll_stp;
if(scroll_pos < scroll_upr) scroll_pos = scroll_upr;
do_scroll(scroll_pos);  
scroll_tim = setTimeout("scroll_up()", scroll_spd);
}

function scroll_dn() {
if(scroll_pos < scroll_lwr) scroll_pos += scroll_stp;
if(scroll_pos > scroll_lwr) scroll_pos = scroll_lwr;
do_scroll(scroll_pos);
scroll_tim = setTimeout("scroll_dn()", scroll_spd);
}

function do_scroll(pos) {
	if(scroll_iex) document.all.divMen.style.top = pos;
	if(scroll_nav) document.divMen.top = pos;
	CurrentStartTop = StartTop + pos;
	scroll_pos = pos;

	if (pos!= scroll_lwr && pos!= scroll_upr && scroll_flagimg!=1) {
		document.images["scroll_down"].src = scroll_img["scroll_down"].src;
		document.images["scroll_end"].src = scroll_img["scroll_end"].src;
		document.images["scroll_up"].src = scroll_img["scroll_up"].src;
		document.images["scroll_start"].src = scroll_img["scroll_start"].src;
		scroll_flagimg = 1;
	}	

	if (pos== scroll_lwr && scroll_flagimg!=2) {
		document.images["scroll_down"].src = scroll_img["scroll_down"].src;
		document.images["scroll_end"].src = scroll_img["scroll_end"].src;
		document.images["scroll_up"].src = scroll_img["scroll_up_d"].src;
		document.images["scroll_start"].src = scroll_img["scroll_start_d"].src;
		scroll_flagimg = 2;
	}
	
	if (pos== scroll_upr && scroll_flagimg!=3) {
	
		document.images["scroll_down"].src = scroll_img["scroll_down_d"].src;
		document.images["scroll_end"].src = scroll_img["scroll_end_d"].src;
		document.images["scroll_up"].src = scroll_img["scroll_up"].src;
		document.images["scroll_start"].src = scroll_img["scroll_start"].src;
		scroll_flagimg = 3;
	}
...
function scroll_RePos() {
	try {
		
		scroll_upr = -(parseInt(divMen.style.height) - FWinH + divBotHeight + divTopHeight + 2);

		divTop.style.top=0;divTop.style.left=0;divTop.style.width=FWinW;divTop.style.height=divTopHeight;
		divMid.style.top=divTopHeight+1;divMid.style.left=0;divMid.style.width=FWinW;divMid.style.height=document.body.clientHeight-divTopHeight-divBotHeight;
		divBot.style.top=document.body.clientHeight-divBotHeight;divBot.style.left=0;divBot.style.width=FWinW;divBot.style.height=divBotHeight;
		
		if (parseInt(divMen.style.height) < (FWinH - divBotHeight - divTopHeight - 2)) {

			try {
			divBot.document.all["scroll_botones_1"].style.visibility = "hidden";
			divBot.document.all["scroll_botones_2"].style.visibility = "hidden";
			} catch (e) {}
			...
			try {
			divBot.document.all["scroll_botones_1"].style.visibility = "visible";
			divBot.document.all["scroll_botones_2"].style.visibility = "visible";
			} catch (e) {}
		}
...
function scroll_build(Lctn) {
			
			divTop = Lctn.document.createElement("div");
			divTop.style.position="absolute";
			divTop.id="divTop";
			Lctn.document.body.appendChild(divTop);
			divTop.style.backgroundColor = divTop.document.body.bgColor;
			divTop.style.zIndex = 100;
			divTop.innerHTML = Lctn.document.all['divTopContent'].innerHTML;
						
			divMid=Lctn.document.createElement("div");
			divMid.style.position="absolute";
			divMid.id="divMid";
			
			divMen=Lctn.document.createElement("div");
			divMen.style.position="absolute";
			divMen.id="divMen";
													
			var MbC=Lctn.document.createElement("div");MbC.style.position="absolute";MbC.style.visibility="hidden";
			
			divMen.appendChild(MbC);
			divMid.appendChild(divMen);
			Lctn.document.body.appendChild(divMid);
			
			divBot=Lctn.document.createElement("div");
			divBot.style.position="absolute";
			divBot.style.backgroundColor = divTop.document.body.bgColor;
			divBot.id="divBot";
			divBot.innerHTML = Lctn.document.all['divBotContent'].innerHTML;
			Lctn.document.all['divBotContent'].innerHTML = "";
			divBot.style.zIndex = 100;
			Lctn.document.body.appendChild(divBot);
