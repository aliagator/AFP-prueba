...
function rutValidar(val) { //para RutSnd
    ...
    if (!reRut.test(valor)) {
        sntAlert("Formato de RUT invalido");
        return false;
    }
...
function numeroAdvFocus(control, tamano, numDec, sepMil, sepDec, ingresando) {
    ...
    if (ingresando) {
        ...
        if (partes == null) {
            sntAlert("Se ha ingresado un formato numérico erróneo.");
            try {
                control.focus()
            } catch (e) {}
            return false;
...
function openKendoWindow(url, title, height, width, additionalOptions, dialogArguments, modal, onCloseFunction) {
    ...
    container = topWnd.$('<div class="sntPopupWindow" />').attr('id', options.name).appendTo(topWnd.document.body);
