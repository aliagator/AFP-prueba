...
$(document).ready(function () {
	if (typeof(arrayMsgboxes) != "undefined")
	{
	    ...
	    for (i = 0; i < arrayMsgboxes.length; i++) {
            ...
            var styleHeight = control.attr("altura").length > 0 ? control.attr("altura")+'px' : '170px';
            var styleWidth = control.attr("ancho").length > 0 ? control.attr("ancho")+'px' : '500px';

            
            var styleConfig = 'dialogHeight:' + styleHeight + ';dialogWidth:' + styleWidth + ';status:0;resizable:0;scroll:0;help:0;unadorned:0';
            var msgBoxHtml = 'js/' + SV_MSGBOX.replace(/\./g, "_") + '/msgbox.htm';
            
            var dialogArguments = { 
                mMensaje: control.attr("mensaje"), 
                mTipo: control.attr("tipo"), 
                mTitulo: control.attr("titulo"),
                mRetorno: 0,
                mControlId: arrayMsgboxes[i]
            };

	        var wnd = window.showModalDialog(msgBoxHtml, dialogArguments, styleConfig);
