...
function numeroFocus(control, sepMil, sepDec, numDec, ingresando) {
    ...
    if (ingresando) {
        ...
        var partes = /-?(\d*)\D?(\d*)/.exec(salida);
        var enteros = partes[1];
        var decimales = partes[2];

        console.log(partes);

        console.log(control.maxLength);

        if(enteros.length > control.maxLength){
            //salida = enteros.substring(0, control.maxLength) + sepDec + decimales;
            salida = enteros + sepDec + decimales;
        }

        console.log(salida);

        salida = parseFloat(salida.replace(sepDec, ".")).toFixed(numDec);
...
function numeroKeypressAdv(evento, control, tamano, numDec, sepDec, aceptaNeg) {
    //var key = (MSIE) ? window.event.keyCode : evento.which;
    ...
    if (key >= 48 && key <= 57) {
        ...
        ok_enteros = (tamano == 0) || (tamano > 0 && partes[1].length <= (tamano));
