...
function verificar_volver() {
    var volver = true;
    if (typeof (SONDANET_PermitirVolverJavascript) != "undefined") if (SONDANET_PermitirVolverJavascript) {
        volver = false;
    }
    if (volver) {
        history.go(1);
...
function ValidarFecha(val) {
    var ctv = $(val).attr("controltovalidate");
...
function rutValidar(val) { //para RutSnd
    ...
    if (dc.toUpperCase() == drut.toUpperCase()) {
        control = $('#' + $(val).attr("controltovalidate"))[0];
...
function reloadIFrame(iframe, url) {
    ...
    if (!url.match(/-=ABORT=-/)) {
		...
		try {	fbody = fbody || ff['body'];																} catch (e) {};
        try {	if (ff['Body']) fbody = fbody || ff['Body'].frames['iframe_body'];							} catch (e) {};
        try {	fbody = fbody || ff['Body'];																} catch (e) {};
        try {	if (ff['side']) fbody = fbody || ff['side'].frames['iframe_body'];							} catch (e) {};
        try {	if (ff['header']) fbody = fbody || ff['header'].frames['iframe_body'];						} catch (e) {};
		try {	if (ff[iframe]) fbody = fbody || window.top;												} catch (e) {};
		try {	if (ff['iframe_body']) fbody = ff['iframe_body'] || window.top;								} catch (e) {};
		
        if (fbody) {
            fbody.frames[iframe].location.href = url;
        } else {
            alert("La funcion reloadIFrame no pudo encontrar el frame interno '" + iframe + "'");
        }
    }
}
