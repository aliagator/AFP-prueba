...
if (document.getElementById) {
    var AgntUsr = navigator.userAgent.toLowerCase(), AppVer = navigator.appVersion.toLowerCase(),
            Opr7 = AgntUsr.indexOf("opera") != -1 && parseInt(AgntUsr.substring(AgntUsr.indexOf("opera") + 6)) > 6
                                                   ? true : false;

    if ((AgntUsr.indexOf("opera") == -1 || Opr7)) {
        var NavYes = ((AgntUsr.indexOf("mozilla") != -1 && AgntUsr.indexOf("compatible") == -1) || Opr7)
                ? true : false, ExpYes = (AgntUsr.indexOf("msie") != -1 && !Opr7) ? true : false;

        if (MenuUsesFrames && !parent.frames[0]) MenuUsesFrames = 0;

        var Exp6Plus = ((AppVer.indexOf("msie 6") != -1 || AppVer.indexOf("msie 7") != -1) && !Opr7)
                ? true : false, MacExp5 = (AppVer.indexOf("mac") != -1 && AppVer.indexOf("msie 5") != -1)
                ? true : false, FLoc, ScLoc, DcLoc, SLdAgnWin, FWinH, FColW, SColW, DColW, SWinW, SWinH, FWinW,
                RLvl = 0, FntFc = 1, StaticPos = 0, Ztop = 100, FrstCreat = true, Ldd = false, Crtd = false,
                FCmplnt = false, SCmplnt = false, M_Clckd = false, IniFlg = false, ShwFlg = false, AcrssFrms = true,
                FrstCntnr = null, CurOvr = null, FHtml = null, ScHtml = null, CloseTmr = null, OpnTmr = null,
                RmbrNow = null, FStr = "", M_Hide = "hidden", M_Show = "visible", P_X = "px", M_StrtTp = StartTop,
                M_StrtLft = StartLeft, Par = MenuUsesFrames ? parent : window, Doc = Par.document, Bod = Doc.body, Trigger = NavYes
                ? Par : Bod, Nav_Op = NavYes
                && AgntUsr.indexOf("gecko/") != -1
                                                    && AgntUsr.indexOf("mac") == -1
                                                                                     && parseInt(AgntUsr.substring(AgntUsr.indexOf("gecko/")
                                                                                                                       + 6,
                                                                                                                   AgntUsr.indexOf("gecko/") + 12))
                                                                                            >= 200303
                                                                                     ? true : false, InitLdd, MstSz = 0;
        LdTmr = setInterval("ChckInitLd()", 100)
    }
}

function ChckInitLd() {
    InitLdd = ExpYes ? MenuUsesFrames ? Par.frames[FirstLineFrame].document.readyState == "complete"
        && Par.frames[SecLineFrame].document.readyState == "complete" ? 1 : 0
        : Par.document.readyState == "complete" ? 1 : 0
        : MenuUsesFrames ? Par.frames[FirstLineFrame].document.body && Par.frames[SecLineFrame].document.body
        ? 1 : 0 : Par.document.body ? 1 : 0;
    ...
    if (!Ldd) {
        ...
        if (ScaleMenu) {
            if (document.defaultView && document.defaultView.getComputedStyle)
                FntFc = parseInt(document.defaultView.getComputedStyle(document.body, '').getPropertyValue("font-size"))
                            / 16;
        ...
        if (FrstCreat || FLoc == ScLoc) FrstCntnr = CreateMenuStructure("Menu", NoOffFirstLineMenus, null);
            ...
            AGREGADO: */
        if(FrstCreat||FLoc==ScLoc) FrstCntnr=CreateMenuStructure("Menu",NoOffFirstLineMenus,null,DivMenu);
		/*  FIN - CAMBIO CODIGO */
...
function SizeMenu(C) {
    var Cmplnt = C.Lvl == 1 ? FCmplnt : SCmplnt, M = C.FrstMbr, Mi, i, W = 0, H = 0, Wd, Ht, RI, BRW = C.Lvl == 1
            ? BorderWidthMain : BorderWidthSub, BTWn = C.Lvl == 1 ? BorderBtwnMain : BorderBtwnSub;

    if (C.Lvl <= FirstLineHorizontal) {
        while (M != null) {
            W += Math.round((M.Arr[5] < 1 ? (M.Arr[5] * (M.Lvl == 1 ? FWinW : SWinW)) : (M.Arr[5] * FntFc)));
            Mi = M;
            M = M.PrvMbr
        }

        W += ((C.NrItms - 1) * BTWn + 2 * BRW);
        Ht = H = Math.round((Mi.Arr[4] < 1 ? (Mi.Arr[4] * (Mi.Lvl == 1 ? FWinH : SWinH)) : (Mi.Arr[4] * FntFc)));
        H += (2 * BRW)
    } else {
        while (M != null) {
            H += Math.round((M.Arr[4] < 1 ? (M.Arr[4] * (M.Lvl == 1 ? FWinH : SWinH)) : (M.Arr[4] * FntFc)));
            Mi = M;
            M = M.PrvMbr
        }

        H += ((C.NrItms - 1) * BTWn + 2 * BRW);
        Wd = W = Math.round((Mi.Arr[5] < 1 ? (Mi.Arr[5] * (Mi.Lvl == 1 ? FWinW : SWinW)) : (Mi.Arr[5] * FntFc)));
        W += (2 * BRW)
    }
    ...
    while (M != null) {
        if (M.Arr[5] < 1) {
            W = C.Lvl <= FirstLineHorizontal ? (M.Arr[5] * (M.Lvl == 1 ? FWinW : SWinW)) : Wd;
            MstSz = 1
        } else W = (C.Lvl <= FirstLineHorizontal ? M.Arr[5] : Wd) * FntFc;

        if (M.Arr[4] < 1) {
            H = C.Lvl <= FirstLineHorizontal ? Ht : (M.Arr[4] * (M.Lvl == 1 ? FWinH : SWinH));
            MstSz = 1
        ...
        if (M.value.indexOf("<") == -1) {
            M.tpd = TopPaddng < 0 ? (Math.round(H * .5 - M.Arr[12] * FntFc)) : TopPaddng;
...
function PosMenu(Ct, Tp, Lt) {
    RLvl++;
    var a, Mb = Ct.FrstMbr, MWt = parseInt(Mb.style.width) + Mb.lx, MHt = parseInt(Mb.style.height)
            + Mb.tx, CWt = parseInt(Ct.style.width), CHt = parseInt(Ct.style.height), CCw, CCh, STp, SLt, Ti, Li,
            Hi, BRW = RLvl == 1 ? BorderWidthMain : BorderWidthSub, BTWn = RLvl == 1
            ? BorderBtwnMain : BorderBtwnSub;

    if (RLvl == 1 && AcrssFrms) !MenuFramesVertical ? Tp = BottomUp ? DistFrmFrameBrdr : FWinH - CHt - DistFrmFrameBrdr
    ...
    /*  INI - CAMBIO CODIGO
        ORIGINAL:
    Ct.style.top = Ct.mif.style.top = RLvl == 1 ? Tp + P_X : 0;
    Ct.OrgTop = Tp;
    Ct.style.left = Ct.mif.style.left = RLvl == 1 ? Lt + P_X : 0;
    Ct.OrgLeft = Lt;
        AGREGADO: */
    if (Ct.id != DivMenu || FrstCreat) {
        Ct.style.top = Ct.mif.style.top = RLvl == 1 ? Tp + P_X : 0;
        Ct.OrgTop = Tp;
        Ct.style.left = Ct.mif.style.left = RLvl == 1 ? Lt + P_X : 0;
        Ct.OrgLeft = Lt;
        if (FrstCreat) FrstCreat = false;
    }
    /*  FIN - CAMBIO CODIGO */
    ...
    while (Mb != null) {
        ...
        if (Mb.CCn) {
            ...
            if (Hi) {
                ...
                STp = RLvl == 1 && AcrssFrms ? BottomUp ? Ti - CCh + MHt : Ti : BottomUp
                    ? Ti - CCh + (1 - ChildVerticalOverlap) * MHt + 2 * BRW : Ti + ChildVerticalOverlap * MHt
            }
        ...
        if (Mb.ai) {
            /*  INI - CAMBIO CODIGO
                ORIGINAL:
            Mb.ai.style.top = (RLvl <= FirstLineHorizontal ? BottomUp ? 2 : MHt - Arrws[a + 2] - 2 : (MHt - Arrws[a + 2]) / 2) + P_X;
            Mb.ai.style.left = (RightToLeft ? 2 : MWt - Arrws[a + 1] - 2) + P_X

                AGREGADO: */
            Mb.ai.style.top = (RLvl <= FirstLineHorizontal ? BottomUp ? 2 : MHt - Arrws[a + 2] - 2 : (MHt - Arrws[a + 2]) / 2) - 6 + P_X;
            Mb.ai.style.left = (RightToLeft ? 2 : MWt - Arrws[a + 1] - 2) - 12 + P_X
            /*  FIN - CAMBIO CODIGO */
        }
...
function HiliteItem(P) {
    ...
    if (P.ro) {
        ...
        if (HooverVariant) P.style.fontVariant = "small-caps";
            ...
            AGREGADO: */
        P.className = typeof(P.Arr[7]) != "undefined" &&  P.Arr[7].length > 0 ? P.Arr[7] : P.Lvl == 1 ? CssClassMainHi : CssClassHi;
        /*  FIN - CAMBIO CODIGO */
    }
...
function LowItem(P) {
    ...
    if (P.ro) {
        ...
        if (HooverVariant) P.style.fontVariant = "normal"
            ...
            AGREGADO: */
        P.className = typeof(P.Arr[6]) != "undefined" &&  P.Arr[6].length > 0 ? P.Arr[6] : P.Lvl == 1 ? CssClassMain : CssClass;
        /*  FIN - CAMBIO CODIGO */
    }
}
...
function OpenMenuII(Ptr) {
    ...
    if (Ptr.Arr[3] && !Ptr.CCn) entrar = 1;
	if (Ptr.CCn && Ptr.CCn.innerHTML=="") entrar = 1;
	...
	/*  FIN - CAMBIO CODIGO */
        ...
        SizeMenu(Ptr.CCn);
        var Ti, Li, Hi, CCw, CCh, STp, SLt, MWt = parseInt(Ptr.style.width) + Ptr.lx, MHt = parseInt(Ptr.style.height)
                + Ptr.tx, BRW = RLvl == 1 ? BorderWidthMain : BorderWidthSub;
        ...
        if (Hi) {
            ...
            STp = RLvl == 1 && AcrssFrms
                ? BottomUp ? Ti - CCh + MHt : Ti : BottomUp ? Ti - CCh + (1 - ChildVerticalOverlap) * MHt + 2 * BRW
                : Ti + ChildVerticalOverlap * MHt + BRW
        }
...
function Unfld() {
    var P = CurOvr, CSt = P.Ctnr.style, CCnt = P.CCn, CCSt = CCnt.style, CCmSt = CCnt.mif.style,
            THt = parseInt(P.style.height), TWt = parseInt(P.style.width), TLt = AcrssFrms && P.Lvl == 1
            && !FirstLineHorizontal ? 0 : parseInt(CSt.left), TTp = AcrssFrms && P.Lvl == 1 && FirstLineHorizontal
            ? 0 : parseInt(CSt.top), CCW = parseInt(CCSt.width), CCH = parseInt(CCSt.height), TS = ExpYes ? SCmplnt
            ? ScHtml.scrollTop : ScLoc.document.body.scrollTop : ScLoc.pageYOffset, LS = ExpYes
            ? SCmplnt ? ScHtml.scrollLeft : ScLoc.document.body.scrollLeft
            : ScLoc.pageXOffset, SLt = AcrssFrms && P.Lvl == 1
            ? CCnt.OrgLeft + TLt + LS : CCnt.OrgLeft + TLt, STp = AcrssFrms && P.Lvl == 1
            ? CCnt.OrgTop + TTp + TS : CCnt.OrgTop + TTp;
    ...
    CCSt.top = CCmSt.top = STp + P_X;
    CCSt.left = CCmSt.left = SLt + P_X;
...
/*  INI - CAMBIO CODIGO
    ...
    AGREGADO: */
    this.className = typeof(this.Arr[6]) != "undefined" &&  this.Arr[6].length > 0 ? this.Arr[6] : this.Lvl == 1 ? CssClassMain : CssClass;
/*  FIN - CAMBIO CODIGO */
...
function CreateMenuStructure(MNm, No, Mcllr) {
    ...
    AGREGADO: */
function CreateMenuStructure(MNm, No, Mcllr, divId) {
/*  FIN - CAMBIO CODIGO */
    RLvl++;

    /*  INI - CAMBIO CODIGO
        ELIMINADO:
    var i, Mbr, PMb = null, WMnu = MNm + "1", MWd = eval(WMnu + "[5]") * FntFc,
     MHt = eval(WMnu + "[4]") * FntFc, Lctn = RLvl == 1 ? FLoc : ScLoc,
     MbC = Lctn.document.createElement("div"),
     MIF = !ExpYes || MacExp5 || !OverFormElements ? MbC : Lctn.document.createElement("iframe");
    Lctn.document.body.appendChild(MbC);

        AGREGADO: */
    var i, Mbr, PMb=null, WMnu=MNm+"1", MWd = eval(WMnu + "[5]") * FntFc,
      MHt = eval(WMnu + "[4]") * FntFc, Lctn = RLvl == 1 ? FLoc : ScLoc;
	var MbC = document.getElementById(divId) ? document.getElementById(divId) : Lctn.document.createElement("div");
	if (divId) MbC.id = divId;
	else MbC.className = CssClassPanel;
	var MIF=!ExpYes || MacExp5 || !OverFormElements ? MbC : Lctn.document.createElement("iframe");
	if (!document.getElementById(divId)) Lctn.document.body.appendChild(MbC);
    /*  FIN - CAMBIO CODIGO */

    if (ExpYes && !MacExp5 && OverFormElements) {
        MIF.src = "blank.html";
    ...
    for (i = 1; i < No + 1; i++) {
        ...
        if (Mbr.Arr[3] && !BuildOnDemand) {
            Mbr.CCn = CreateMenuStructure(WMnu + "_", Mbr.Arr[3], Mbr)
        }
        PMb = Mbr
    }
    MbC.FrstMbr = Mbr;
